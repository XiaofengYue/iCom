...
def get_type_page():
    try:
        ...
        p_type = p_type.split(' ')
        type_rule = or_(*[Comtype.comtype_typename.like(w) for w in p_type])
        all_compe = Competition.query.filter(type_rule).join(Comtype, Comtype.comtype_comid == Competition.com_id).order_by(Competition.com_id.desc()).limit(p_pageSize).offset((p_page - 1) * p_pageSize).all()
        return return_json(data=[comp.to_dict() for comp in all_compe])
    except Exception:
        return return_json(code=0, msg='请求参数有误')
